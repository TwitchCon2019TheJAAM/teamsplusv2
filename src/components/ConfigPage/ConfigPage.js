    ...
    render() {
        if (true){//(this.state.finishedLoading && this.Authentication.isModerator()) {
            const trustedList = this.state.trustedMembers.map(member => {
                return <li className="viewer"><input className="checkBox" type="checkbox" checked onChange={event => this.updateTrust(member.name,event.target.checked)}></input>{member.name}</li>
            })
            const viewerList = this.state.currentViewers.map(viewer => {
                return <li className="viewer"><input className="checkBox" type="checkbox" onChange={event => this.updateTrust(viewer.name,event.target.checked)}></input>{viewer.name}</li>
            })
            return (
